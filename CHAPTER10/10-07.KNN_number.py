...
train_image = cv2.imread("images/train_number.png", cv2.IMREAD_GRAYSCALE)
...
print("nums 형태:", np.array(cells).shape)
print("trainData 형태:", trainData.shape)
print("labels 형태:", labels.shape)

knn = cv2.ml.KNearest_create()
knn.train(trainData, cv2.ml.ROW_SAMPLE, classLabel)

plt.figure(figsize=(10, 10))
for i in range(50):
    test_img = cv2.imread("images/num/%d%d.png" % (i / 5, i % 5), 0)
    cv2.threshold(test_img, 128, 255, cv2.THESHOLD_BINARY, test_img)
